...
class ClientInfo:
    ...
    # [NEW 2026-02-03] CLI 工具使用无状态模式，绕过 SCID 架构和签名恢复
    # CLI 工具（如 Claude Code, Cline, Aider）有自己的状态管理，不需要服务端状态
    is_stateless: bool = False        # 是否使用无状态模式（CLI 工具）
    # [NEW 2026-02-04] 轻量级签名恢复模式（跳过完整 SCID 状态管理，但保留签名恢复）
    # 用于 Claude Code 等需要签名恢复但不需要完整状态管理的客户端
    needs_signature_recovery_only: bool = False
...
class ClientTypeDetector:
    ...
    def detect(cls, headers: Dict[str, str]) -> ClientInfo:
        ...
        # 6. [NEW 2026-02-03] 判断是否使用无状态模式（CLI 工具）
        is_stateless = client_type in cls.STATELESS_CLIENTS

        # 7. [NEW 2026-02-04] 判断是否需要轻量级签名恢复（跳过完整 SCID 状态管理）
        needs_signature_recovery_only = client_type in cls.SIGNATURE_RECOVERY_ONLY_CLIENTS
        ...
        client_info = ClientInfo(
            ...
            needs_signature_recovery_only=needs_signature_recovery_only,
        )
        ...
        log.info(
            ...
            f"stateless={is_stateless}, sig_recovery_only={needs_signature_recovery_only}, "
