...
class IDECompatMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            # 4. 读取请求体 - 可能抛出 ClientDisconnectedError
            try:
                body = await self._read_body(request)
            except ClientDisconnectedError:
                # [FIX 2026-02-07] 客户端断开连接，立即返回空响应
                # 不能继续调用 call_next，因为客户端已经不在了
                log.warning(
                    "[IDE_COMPAT_MIDDLEWARE] Client disconnected, returning empty response"
                )
                self.stats["errors"] += 1
                return Response(
                    content="",
            ...
            # [FIX 2026-02-07] 顶层捕获，确保不会遗漏
            log.warning(
                "[IDE_COMPAT_MIDDLEWARE] Client disconnected at top level"
            )
            self.stats["errors"] += 1
            return Response(
                content="",
    ...
    async def _read_body(self, request: Request) -> Optional[Dict[str, Any]]:
        ...
        Raises:
            ClientDisconnectedError: 当客户端断开连接时抛出
        """
        try:
            # 读取原始请求体
            ...
            # [FIX 2026-02-07] 客户端断开连接 - 这是一个需要特殊处理的错误
            # 不能继续调用 call_next，必须立即终止处理
            log.warning(
                "[IDE_COMPAT_MIDDLEWARE] Client disconnected before body could be read"
            )
            raise ClientDisconnectedError("Client disconnected during body read")
        except json.JSONDecodeError as e:
            log.warning(f"[IDE_COMPAT_MIDDLEWARE] Failed to parse JSON body: {e}")
            return None
        except Exception as e:
            # 检查是否是 ClientDisconnect 的变体
            if "ClientDisconnect" in type(e).__name__ or "Disconnect" in str(type(e)):
                log.warning(
                    "[IDE_COMPAT_MIDDLEWARE] Client disconnected (caught as Exception)"
                )
                raise ClientDisconnectedError("Client disconnected during body read")
