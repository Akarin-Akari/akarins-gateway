...
# 导入签名处理相关模块
from akarins_gateway.converters.thoughtSignature_fix import (
...
from akarins_gateway.converters.signature_recovery import (
...
# [FIX 2026-02-04] 导入 thinking 规范化模块
try:
    from akarins_gateway.thinking_normalizer import (
        normalize_messages_for_cache,
        ENABLE_THINKING_NORMALIZATION,
        NORMALIZATION_LEVEL
    )
    THINKING_NORMALIZER_AVAILABLE = True
except ImportError:
    THINKING_NORMALIZER_AVAILABLE = False
    ENABLE_THINKING_NORMALIZATION = False
...
class AnthropicSanitizer:
    ...
    def sanitize_messages(
        ...
        try:
            ...
            # [FIX 2026-02-04] 应用 thinking 规范化以提高 Prompt Cache 命中率
            if THINKING_NORMALIZER_AVAILABLE and ENABLE_THINKING_NORMALIZATION:
                try:
                    sanitized_messages = normalize_messages_for_cache(
                        sanitized_messages,
                        level=NORMALIZATION_LEVEL
                    )
                    log.debug(
                        f"[SANITIZER] Applied thinking normalization: "
                        f"level={NORMALIZATION_LEVEL}, messages={len(sanitized_messages)}"
                    )
                except Exception as norm_err:
                    log.warning(f"[SANITIZER] Thinking normalization failed (non-fatal): {norm_err}")
    ...
    def _validate_thinking_block(
        ...
        try:
            ...
            if recovery_result.signature and is_valid_signature(recovery_result.signature):
                ...
                # [FIX 2026-01-24] 记录签名恢复成功
                try:
                    from akarins_gateway.ext_thinking_metrics import get_metrics_collector
                    get_metrics_collector().record_signature_recovery(
                        success=True,
                        layer=recovery_result.source.value
                    )
                except Exception:
                    pass
                ...
                # [FIX 2026-01-24] 记录签名恢复失败
                try:
                    from akarins_gateway.ext_thinking_metrics import get_metrics_collector
                    get_metrics_collector().record_signature_recovery(success=False)
                    get_metrics_collector().record_thinking_downgrade(removed=False)
                except Exception:
                    pass
    ...
    def _sync_thinking_config(
        ...
        规则:
        - [FIX 2026-02-12] 客户端明确请求 thinking 时,即使没有历史 thinking block 也保持启用
        - 如果 thinking_enabled=False,确保消息中没有 thinking block

        Args:
            messages: 消息列表
            thinking_enabled: 当前 thinking 配置 (来自客户端请求)
        ...
        # 同步逻辑
        # [FIX 2026-02-12] 修复 thinking 未启用的严重 bug
        # 原逻辑: thinking_enabled=True 但没有 thinking block → 禁用 thinking
        # 这会导致新对话/清理后的对话永远无法启用 thinking (鸡生蛋问题)
        # 修复: 客户端明确请求 thinking 时，即使历史消息中没有 thinking block 也应保持启用
        # 只有在 thinking 未被请求但消息中存在 thinking block 时才需要同步处理
        if thinking_enabled and not has_valid_thinking:
            log.debug(
                "[SANITIZER] No valid thinking blocks in messages, "
                "but thinking was explicitly requested by client - keeping enabled"
            )
            # 保持 thinking_enabled=True，不再错误地禁用
            return True
    ...
    def _cache_recovered_signature(
        ...
        try:
            # 导入缓存函数 (延迟导入避免循环依赖)
            from akarins_gateway.signature_cache import cache_signature, cache_session_signature
