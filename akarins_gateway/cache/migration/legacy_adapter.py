...
class LegacySignatureCacheAdapter:
    ...
    def set(
        ...
        Args:
            ...
            model: 可选的模型名称
            conversation_id: 可选的会话标识（用于新缓存隔离）
        ...
        if self._flags.should_write_to_new:
            new_writer = lambda text, sig, mdl: self._write_to_new(
                text, sig, mdl, conversation_id
            )
    ...
    def get(self, thinking_text: str, conversation_id: Optional[str] = None) -> Optional[str]:
        """
        获取缓存的 signature
        ...
        # 使用读取策略
        new_reader = None
        if self._flags.should_read_from_new:
            new_reader = lambda text, namespace: self._read_from_new(
                text, namespace, conversation_id
            )
...
def cache_signature_v2(
    ...
    Returns:
        是否成功缓存
    """
    return get_legacy_adapter().set(thinking_text, signature, model, conversation_id)


def get_cached_signature_v2(
    thinking_text: str,
    conversation_id: Optional[str] = None
) -> Optional[str]:
    """
    ...
    Returns:
        缓存的 signature
    """
    return get_legacy_adapter().get(thinking_text, conversation_id)
