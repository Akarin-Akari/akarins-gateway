...
class SignatureDatabase(ICacheLayer):
    ...
    # [FIX 2026-02-04] Session Checkpoints 表 - 存储中断恢复检查点
    ...
    CREATE_INDEXES_SQL = [
        ...
        # [FIX 2026-02-04] Session Checkpoints 索引
        "CREATE INDEX IF NOT EXISTS idx_checkpoint_scid ON session_checkpoints(scid)",
        "CREATE INDEX IF NOT EXISTS idx_checkpoint_expires ON session_checkpoints(expires_at)",
        "CREATE INDEX IF NOT EXISTS idx_checkpoint_timestamp ON session_checkpoints(timestamp)",
    ]
        ...
        with self._init_lock:
            ...
            try:
                with self._get_cursor() as cursor:
                    ...
                    # [FIX 2026-02-04] Create Session Checkpoints table
                    cursor.execute(self.CREATE_CHECKPOINT_TABLE_SQL)
                ...
                log.info("[SIGNATURE_DB] Database schema initialized (including tool_cache, session_cache, conversation_state, and session_checkpoints)")
    ...
    def get_by_thinking_hash_any_namespace(
        ...
        try:
            with self._get_cursor(commit=False) as cursor:
                ...
                log.info(f"[SIGNATURE_DB] Fallback cache hit: hash={thinking_hash[:16]}...")
                return entry
    ...
    def set_tool_signature(
        ...
        try:
            with self._get_cursor() as cursor:
                ...
                if cursor.rowcount > 0:
                    log.info(f"[SIGNATURE_DB] Tool signature stored: tool_id={tool_id[:20]}...")
                    return True
                return False
    ...
    def get_tool_signature(self, tool_id: str) -> Optional[str]:
        ...
        try:
            with self._get_cursor(commit=False) as cursor:
                ...
                log.info(f"[SIGNATURE_DB] Tool signature hit: tool_id={tool_id[:20]}...")
    ...
    def _update_tool_access_stats(self, tool_id: str) -> None:
        ...
        try:
            ...
            log.warning(f"[SIGNATURE_DB] Error updating tool access stats: {e}")
    ...
    # [FIX 2026-02-04] Session Checkpoint CRUD - 存储中断恢复检查点
    ...
    def save_checkpoint(
        ...
        try:
            with self._get_cursor() as cursor:
                cursor.execute(
                    ...
                    INSERT INTO session_checkpoints (
                        scid, thinking_content, partial_response, signature,
                        timestamp, is_complete, created_at, updated_at, expires_at
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                    ON CONFLICT(scid) DO UPDATE SET
                    ...
                    (
                        scid,
                        checkpoint.get("thinking_content", ""),
                        checkpoint.get("partial_response", ""),
                        checkpoint.get("signature"),
                        checkpoint.get("timestamp", time.time()),
                        1 if checkpoint.get("is_complete", False) else 0,
                        now.isoformat(),
                        now.isoformat(),
                        expires_at,
                    )
                ...
                if cursor.rowcount > 0:
                    log.debug(
                        f"[SIGNATURE_DB] Checkpoint saved: scid={scid[:20]}..., "
            ...
            log.error(f"[SIGNATURE_DB] Error saving checkpoint: {e}")
            return False

    def get_checkpoint(self, scid: str) -> Optional[Dict[str, Any]]:
        """
        从SQLite获取检查点
        ...
        try:
            with self._get_cursor(commit=False) as cursor:
                cursor.execute(
                    """
                    SELECT scid, thinking_content, partial_response, signature,
                           timestamp, is_complete, created_at, updated_at, expires_at
                ...
                if row["expires_at"]:
                    ...
                    if datetime.now() > expires_at:
                        # Delete expired entry
                        self.delete_checkpoint(scid)
                        return None

                log.debug(
                    f"[SIGNATURE_DB] Checkpoint retrieved: scid={scid[:20]}..., "
                    f"is_complete={bool(row['is_complete'])}"
                )
            ...
            log.error(f"[SIGNATURE_DB] Error getting checkpoint: {e}")
            return None

    def delete_checkpoint(self, scid: str) -> bool:
        """
        删除检查点
        ...
        try:
            with self._get_cursor() as cursor:
                cursor.execute(
                    "DELETE FROM session_checkpoints WHERE scid = ?",
                    (scid,)
                )
                if cursor.rowcount > 0:
                    log.debug(f"[SIGNATURE_DB] Checkpoint deleted: scid={scid[:20]}...")
                    return True
                return False

        except Exception as e:
            log.error(f"[SIGNATURE_DB] Error deleting checkpoint: {e}")
            return False
    ...
    def cleanup_expired_checkpoints(self) -> int:
        ...
        try:
            ...
            with self._get_cursor() as cursor:
                ...
                count = cursor.rowcount
                if count > 0:
                    log.info(f"[SIGNATURE_DB] Cleaned up {count} expired checkpoint entries")
                return count

        except Exception as e:
            log.error(f"[SIGNATURE_DB] Error cleaning up expired checkpoints: {e}")
            return 0
