...
class CacheFacade:
    ...
    def cache_signature(
        ...
        return cache.set(thinking_text, signature, model, conversation_id)
    ...
    def get_cached_signature(
        ...
        """
        cache = self._get_active_cache()
        return cache.get(thinking_text, conversation_id)
    ...
    def cache_tool_signature(self, tool_id: str, signature: str) -> bool:
        ...
        try:
            ...
            if result:
                log.info(f"[CACHE_FACADE] Tool signature cached: tool_id={tool_id[:20]}...")
            return result
    ...
    def get_tool_signature(self, tool_id: str) -> Optional[str]:
        ...
        try:
            ...
            if result:
                log.info(f"[CACHE_FACADE] Tool signature hit: tool_id={tool_id[:20]}...")
            return result
        except Exception as e:
...
def cache_signature(
    ...
    """缓存 signature（便捷函数）"""
    return get_cache_facade().cache_signature(
        thinking_text,
        signature,
        model,
        conversation_id
...
def get_cached_signature(
    ...
    return get_cache_facade().get_cached_signature(thinking_text, conversation_id)
