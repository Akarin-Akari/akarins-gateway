...
class RateLimiter:
    ...
    Usage:
        limiter = RateLimiter(min_interval_ms=150)
        await limiter.wait()  # 第一次调用立即返回
        await limiter.wait()  # 第二次调用等待至少 150ms

    [FIX 2026-02-02] 优化：
    - 默认间隔从 500ms 降低到 150ms，减少 Claude Code 高频调用延迟
    - 重构锁机制：锁外 sleep，避免锁竞争导致的阻塞
    """
    ...
    def __init__(self, min_interval_ms: int = 150):
        ...
        Args:
            min_interval_ms: 最小调用间隔（毫秒），默认 150ms
                            [FIX 2026-02-02] 从 500ms 降低到 150ms
        """
    ...
    async def wait(self) -> None:
        ...
        [FIX 2026-02-02] 重构：锁外 sleep，避免锁竞争
        - 之前：在锁内 sleep，导致其他协程被阻塞
        - 现在：只在锁内计算等待时间和更新时间戳，锁外执行 sleep
        """
        ...
        async with self._lock:
            ...
            # 预先更新 last_call，避免其他协程计算出相同的等待时间
            self._last_call = now + sleep_time

        # 锁外执行 sleep，不阻塞其他协程获取锁
        if sleep_time > 0:
...
class AdaptiveRateLimiter:
    ...
    def record_success(self) -> None:
        ...
        log.info(f"[ADAPTIVE_RATE_LIMITER] Success, interval reduced to {self._current_interval * 1000:.0f}ms")
