...
# [FIX 2026-02-01] 延迟导入打破循环：gemini_fix <-> anthropic_converter
# anthropic_converter 导入 gemini_fix (clean_contents, ALLOWED_PART_KEYS)
# gemini_fix 若顶层导入 anthropic_converter 会导致循环
# 改为在 normalize_gemini_request 内需要时再导入
    ...
    if mode == "geminicli":
        ...
        if "image" in model.lower():
            ...
            if is_thinking_model(model):
                ...
                # 优先使用传入的思考预算，否则使用默认值
                # 延迟导入避免 gemini_fix <-> anthropic_converter 循环依赖
                if "thinkingBudget" not in thinking_config:
                    from akarins_gateway.converters.anthropic_constants import DEFAULT_THINKING_BUDGET
                    thinking_config["thinkingBudget"] = DEFAULT_THINKING_BUDGET
                # [FIX 2026-01-17] 移除 thinkingLevel 避免与 thinkingBudget 冲突（官方版本修复）
            ...
            if "opus" in model.lower():
                # ✅ [FIX 2026-02-11] 区分 Opus 4.6 和 4.5，4.6 全面支持所有后端
