...
# [FIX 2026-02-17] Gemini 模型 maxOutputTokens 自适应提升
# 参考 Antigravity-Manager v4.1.14
# 不同 Gemini 模型的 maxOutputTokens 上限（前缀匹配）
...
def get_max_output_tokens(model_name: str, requested: Optional[int] = None) -> int:
    """
    [FIX 2026-02-17] 根据模型获取 maxOutputTokens 上限，使用前缀匹配。

    Args:
        model_name: Gemini 模型名称（如 "gemini-2.5-flash-preview-05-20"）
        requested: 用户请求的 max_tokens 值（None 表示未指定）

    Returns:
        最终的 maxOutputTokens 值
    """
    model_lower = model_name.lower() if model_name else ""
    model_limit = DEFAULT_MODEL_MAX_OUTPUT_TOKENS

    # [FIX 2026-02-17] 按前缀长度降序排序后匹配，防止短前缀遮蔽长前缀
    # 例如 "gemini-2.5-flash-preview-05-20" 应匹配 "gemini-2.5-flash" 而非 "gemini-2.0-flash"
    for prefix in sorted(MODEL_MAX_OUTPUT_TOKENS.keys(), key=len, reverse=True):
        if model_lower.startswith(prefix):
            model_limit = MODEL_MAX_OUTPUT_TOKENS[prefix]
            break

    if requested is None:
        return model_limit

    result = min(requested, model_limit)
    if result != requested:
        log.info(
            f"[MODEL_CONFIG] maxOutputTokens 被模型上限截断: {requested} -> {result} "
            f"(model={model_name}, limit={model_limit})"
        )
    return result
...
def model_mapping(model_name: str) -> str:
    ...
    mapping = {
        ...
        "claude-opus-4-5": "claude-opus-4-5-thinking",
        "gemini-2.5-flash-thinking": "gemini-2.5-flash",
        # ✅ [FIX 2026-02-08] Opus 4.6 映射（自研后端使用 thinking 格式）
        "claude-opus-4.6": "claude-opus-4-6-thinking",
        "claude-opus-4-6": "claude-opus-4-6-thinking",
        # ✅ [FIX 2026-02-11] Cursor / Augment / Claude Code 等 IDE Opus 4.6 客户端变体
        "claude-4.6-opus-high-thinking": "claude-opus-4-6-thinking",
        "claude-4.6-opus-high": "claude-opus-4-6-thinking",
        ...
        "claude-4-6-opus": "claude-opus-4-6-thinking",
        "claude-4-6-opus-thinking": "claude-opus-4-6-thinking",
        "claude-opus-4-6-high-thinking": "claude-opus-4-6-thinking",
        "claude-opus-4-6-high": "claude-opus-4-6-thinking",
        # Cursor 客户端模型名映射 (4.5 系列)
...
def get_fallback_models(model_name: str) -> List[str]:
    ...
    """
    try:
        from akarins_gateway.fallback_manager import get_cross_pool_fallback, is_haiku_model, is_opus_model, HAIKU_FINAL_FALLBACK
    except ImportError:
        # fallback_manager not extracted to akarins-gateway — graceful degradation
        log.debug("[FALLBACK] fallback_manager not available, returning empty fallback list")
        return []
    ...
    if is_opus_model(model_name):
        log.info(f"[FALLBACK] Opus 模型 {model_name} 不在 AG 层跨池降级，由 Gateway 层处理")
...
def should_fallback_on_error(error_msg: str) -> bool:
    ...
    """
    try:
        from akarins_gateway.fallback_manager import is_quota_exhausted_error
    except ImportError:
        # fallback_manager not available — don't trigger fallback
        return False
...
def is_thinking_model(model_name: str) -> bool:
    ...
    [FIX 2026-02-11] Opus 模型天生支持 thinking，无需 -thinking 后缀
    Cursor 发送的 claude-4.6-opus-high 等变体也应被视为 thinking 模型
    """
    # 检查是否包含 -thinking 后缀
    ...
    # ✅ [FIX 2026-02-11] Opus 模型天生支持 extended thinking
    # Opus 4.5+ 默认启用 thinking，无需 -thinking 后缀
