...
__all__ = [
    ...
    # [FIX 2026-02-05] 增量 tool_call 合并
...
def merge_tool_call_deltas(tool_calls: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    合并增量 tool_calls 为完整的工具调用。

    [FIX 2026-02-05] 解决 Cursor 流式响应中 tool_call 被分割的问题。

    问题场景：
    Cursor 可能发送增量的 tool_calls，每个只包含 arguments 片段：
    [
        {"function": {"name": "Read"}, "index": 0, "id": "call_1"},
        {"function": {"arguments": '{"pa'}, "index": 0},
        {"function": {"arguments": 'th": "'}, "index": 0},
        {"function": {"arguments": 'file.txt"}'}, "index": 0},
    ]

    这些增量被错误地当作独立的 tool_calls，导致 Pydantic 验证失败：
    messages.3.tool_calls.1.function.arguments: '{"pa'

    解决方案：
    按 index 分组，累积合并 name 和 arguments。

    Args:
        tool_calls: 可能包含增量片段的 tool_calls 列表
    ...
    # 检测是否存在增量模式（有 index 字段且多个相同 index）
    has_index = any(isinstance(tc, dict) and "index" in tc for tc in tool_calls)
    if not has_index:
        # 没有 index 字段，可能已经是完整的 tool_calls
        return tool_calls
    ...
    for tc in tool_calls:
        ...
        if isinstance(func, dict):
            # 合并 name（取第一个非空的）
            if func.get("name") and not merged[idx]["function"]["name"]:
            ...
            if func.get("arguments"):
                ...
                if isinstance(args, str):
                    merged[idx]["function"]["arguments"] += args
                elif isinstance(args, dict):
                    # 如果 arguments 是字典，需要转换为字符串
                    import json
                    merged[idx]["function"]["arguments"] += json.dumps(args, ensure_ascii=False)

        # 合并 id（取第一个非空的）
        if tc.get("id") and not merged[idx]["id"]:
            merged[idx]["id"] = tc["id"]
    ...
    for idx in sorted(merged.keys()):
        ...
        # 确保有 id
        if not tc["id"]:
            tc["id"] = f"call_{idx}_{tc['function'].get('name', 'unknown')}"
        ...
        if args:
            try:
                ...
                # 如果不是有效 JSON，可能还在累积中，记录警告
                log.warning(f"[TOOL_CALL_MERGE] Invalid JSON in merged arguments for index {idx}: {args[:100]}...")
        result.append(tc)

    if len(result) != len(tool_calls):
        log.info(f"[TOOL_CALL_MERGE] Merged {len(tool_calls)} delta tool_calls into {len(result)} complete tool_calls")
...
def normalize_messages(messages: List[Any]) -> List[Dict[str, Any]]:
    ...
    for msg in messages:
        ...
        if msg.get("role") == "assistant" and msg.get("tool_calls"):
            # [FIX 2026-02-05] 先合并增量 tool_calls，再添加到 pending
            raw_tool_calls = msg.get("tool_calls", [])
            merged_tcs = merge_tool_call_deltas(raw_tool_calls)
            pending_tool_calls.extend(merged_tcs)
...
def normalize_request_body(
    ...
    *,
    ...
    Args:
        ...
        body_from_bridge: Phase 5 - 当 body 来自 Bridge 时，信任 body["messages"]，跳过 Augment 格式解析
    ...
    # [Phase 5] 当 body 来自 Bridge 时，信任 body["messages"]，跳过 Augment 格式解析（避免 "No messages found"）
    if body_from_bridge and isinstance(body.get("messages"), list):
        messages = body["messages"]
    # Standard location (OpenAI / Bridge 输出)
    if messages is None and "messages" in body and body["messages"]:
        messages = body["messages"]
    # Augment Code format: 'message' + 'chat_history'（仅当非 Bridge 路径时解析）
    elif messages is None and "message" in body:
